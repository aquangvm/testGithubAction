...
public interface INodeRepository2 extends IRepository<NodeModel2> {
    ...
    @Query("SELECT node.id FROM BpmnModel2 bpmn2 JOIN NodeModel2 node ON bpmn2.id = node.bpmnId WHERE bpmn2.active = :active AND node.clientId = :clientId and node.active = :active and bpmn2.orgId = :orgId and node.type = 'endEvent' and bpmn2.typeDocument = 'INCOMING'")
