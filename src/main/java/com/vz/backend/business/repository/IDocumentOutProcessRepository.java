...
public interface IDocumentOutProcessRepository extends IRepository<DocumentOutProcess> {
	...
	@Query("SELECT new com.vz.backend.business.dto.DocumentProcessDto(p.documentOut.id, du.important, p.id, p.documentOut.numberOrSign, "
			...
			+ " p.handler.id, p.handler.userName, p.handler.fullName, p.updateDate, p.documentOut.security.id, p.documentOut.security.name, p.documentOut.security.categoryTypeId, "
			+ " p.handleStatus, p.documentOut.nodeId, p.documentOut.listSignerIds, u1.id, u1.userName, u1.fullName, u2.id, u2.userName, u2.fullName, p.documentOut.status, p.documentOut.numberInBook, p.read) "
