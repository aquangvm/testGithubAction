...
public interface IDelegateRepository extends IRepository<Delegate> {
	...
	@Query("SELECT new com.vz.backend.business.dto.DocumentProcessDto(p.documentOut.id, du.important, p.id, p.documentOut.numberOrSign, "
			+ " p.documentOut.docType.id, p.documentOut.docType.name, p.documentOut.docType.categoryTypeId,"
			+ " p.documentOut.docFieldId, p.documentOut.preview, p.documentOut.userEnter.id, p.documentOut.userEnter.userName, p.documentOut.userEnter.fullName, p.documentOut.createDate, "
			+ " p.user.id, p.user.userName, p.user.fullName, p.updateDate, p.documentOut.security.id, p.documentOut.security.name, p.documentOut.security.categoryTypeId, "
			+ " p.handleStatus, p.documentOut.nodeId, p.documentOut.listSignerIds, p.documentOut.numberInBook, p.read) " + "FROM DocumentOutProcess p "
	...
	@Query("SELECT new com.vz.backend.business.dto.UserDelegateDto(de.id, de.endDate, de.fromUserId, de.toUser.id, de.toUser.userName, de.toUser.fullName, de.toUser.positionModel.name, de.toUser.positionModel.order) "
