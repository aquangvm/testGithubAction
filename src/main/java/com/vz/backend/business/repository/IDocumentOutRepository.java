...
public interface IDocumentOutRepository extends IRepository<DocumentOut> {
	...
	Page<DocumentOut> getListDocAdvance(String text, boolean clericalOrg, DocumentTypeEnum docType, Boolean important,
			...
			Long bookId, DocumentStatusEnum[] status, Long clientId, Long orgId, Pageable castToPageable);
	...
	@Query("SELECT new com.vz.backend.business.dto.DocOutDto(0, d.id, d.numberOrSign, d.preview, c.name, d.userEnter.userName, d.createDate, d.dateIssued, d.urgentId, d.securityId,d.numberInBook) "
	...
	@Query("SELECT new com.vz.backend.business.dto.DocOutDto(0, d.id, d.numberOrSign, d.preview, c.name, d.userEnter.userName, d.createDate, d.dateIssued, d.urgentId, d.securityId,d.numberInBook) "
			...
			+ "WHERE (:text is null OR lower(d.numberOrSign) like %:text% OR (concat(d.numberInBook,lower(d.numberOrSign))) like %:text%  OR lower(d.preview) like %:text%) "
	...
	Page<DocOutDto> getListIssued(boolean clericalOrg, String text, Long userId, Long orgId, Long clientId, DocumentStatusEnum status,
			Pageable castToPageable);
	...
	@Query("SELECT new com.vz.backend.business.dto.DocOutDto(0, d.id, d.numberOrSign, d.preview, c.name, d.userEnter.userName, d.createDate, d.dateIssued, d.urgentId, d.securityId,d.numberInBook) "
			...
			+ "OR (concat(d.numberInBook,lower(d.numberOrSign))) like %:numberOrSign%) "
			+ "AND (:orgCreateName is null or lower(d.orgCreateName) = :orgCreateName) "
			...
			+ "AND ((:clericalOrg is true AND d.orgIssuedId = co.orgId) OR (:clericalOrg is false AND d.orgIssuedId = :orgId)) "
			+ "AND d.clientId = :clientId AND d.status = :status AND d.active = TRUE ")
	Page<DocOutDto> getListIssued(boolean clericalOrg, String numberOrSign, String orgCreateName, String personEnter, String preview,
			Date startCreate, Date endCreate, Date startIssued, Date endIssued, Long docTypeId, Long docFieldId,
	...
	Page<DocumentOutDto> getListDocProcessAdvanceAssign(String text, DocumentTypeEnum docType, Boolean important,
			...
			DocumentStatusEnum[] docStatus, Long clientId, Pageable castToPageable);
	...
	Page<KnowableDto> findKnowable(DocumentTypeEnum docType, Long userId, DocumentStatusEnum status, List<Long> docIds, boolean done, Pageable pageable);
	...
	@Query("SELECT new com.vz.backend.business.dto.KnowableDto(d.id, d.numberInBook, d.numberOrSign, d.docType.id, d.docType.name, d.userEnter.fullName, d.dateIssued, d.preview, d.status, du.important, ob.read, dr.handleType) "
			...
			+ "LEFT JOIN User u ON (r.type ='ORG' OR r.type = 'ALL') AND r.receiveId=u.org AND u.id=:userId "
			+ "LEFT JOIN Category ca ON u.position=ca.id "
			+ "INNER JOIN User u2 ON d.personEnterId=u2.id " + "WHERE d.status=:status AND d.clientId=:clientId AND (:handleType IS NULL OR r.handleType = :handleType) AND "
			+ "((r.type ='FORWARD' AND r.receiveId=:userId) OR (r.type ='USER' AND r.receiveId=:userId) OR (r.type ='ORG' AND ca.isLeadership is TRUE AND ca.isSiblings is TRUE AND r.receiveId=u.org ) OR (r.type ='ALL' AND r.receiveId=u.org ))"
			+ "AND (:preview IS NULL OR lower(d.preview) LIKE %:preview%)"
			+ "AND (:numberOrSign IS NULL OR lower(d.numberOrSign) LIKE %:numberOrSign%)"
	...
	public static final String FIND_ALL_QUERY = 
			...
			+ "  )"
//					+ "  OR ((:lead = TRUE) AND d.id IN (SELECT dr2.docId FROM DocumentReceive dr2 WHERE dr2.receiveId = :orgId AND dr2.type = 'ORG' AND dr2.active = TRUE AND dr2.clientId=:clientId))"
	...
	Page<DocumentOut> getListAllAdvance(Date date, String text, boolean clericalOrg, String outsideReceive,
			...
			Long docTypeId, Long docFieldId, Long bookId, Long clientId, List<Long> docIds, Pageable castToPageable);
	...
	List<DocumentOut> getListAllAdvanceNotPaging(Date date, String text, boolean clericalOrg, String outsideReceive,
			Boolean important, Long orgId, String personEnter, Long userId, boolean vanThu, String numberOrSign,
			String preview, String orgCreateName, Date startCreate, Date endCreate, Date startIssued, Date endIssued,
			Long docTypeId, Long docFieldId, Long bookId, Long clientId, List<Long> docIds);
	...
	Page<DocumentOut> findAll(String numberOrSign, String preview, Long docFieldsId, DocumentStatusEnum docStatusId,
			Long urgentId, Long securityId, Long userId, Long orgIssuedId, Date createFrom, Date createTo, Date issuedDateFrom, Date issuedDateTo, Long clientId, Pageable castToPageable);
...
//			+ " AND (:docTypeId is null OR d.docTypeId = :docTypeId) "
	...
	List<DocumentOut> getFollowDocumentVTBBanHanh(String numberOrSign, Long bookId,
												  String preview, Long docFieldsId,
												  DocumentStatusEnum docStatusId, Long urgentId, Long orgIssuedId, Date createFrom,Date createTo,
...
//			+ " AND (:docTypeId is null OR d.docTypeId = :docTypeId) "
	...
	List<DocumentOut> getFollowDocumentLeadBan(String numberOrSign, Long bookId,
											   String preview, Long docFieldsId,
											   DocumentStatusEnum docStatusId, Long urgentId,
											   Long orgIssuedId, Date createFrom,Date createTo,
	...
	List<Long> signerDocumentOutSearch(Long userId, Long clientId, String preview, String numberOrSign, String orgName, Long docFieldId,
	...
	List<DocumentOut> exportAllDoc(String numberOrSign,
								   ...
								   List<Long> orgIds,DocumentStatusEnum status,Long bookId,Long urgentId,Long securityId,Long orgIssuedId);
	@Query(value = "select d from DocumentOut d"
			...
			+ " AND (:docTypeId is null OR d.docTypeId = :docTypeId) "
			+ " and (d.clientId = :clientId ) ")
	List<DocumentOut> exportFollowDocumentVTBBanHanh(String numberOrSign, String preview, Date createFrom,Date createTo,Long docFieldsId,
													 Long clientId, Long userId, DocumentTypeEnum docType ,DocumentStatusEnum status,Long bookId,Long urgentId,Long securityId,Long orgIssuedId,Long docTypeId);
	...
	List<DocumentOut> exportFollowDocumentLeadBan(String numberOrSign,
												  ...
												  List<Long> userId, DocumentTypeEnum docType,DocumentStatusEnum status,Long bookId,Long urgentId,Long securityId,Long orgIssuedId,Long docTypeId);
}
