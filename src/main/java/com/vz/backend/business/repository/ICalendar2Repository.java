...
//			+ " AND ((:status) IS NULL OR c.status IN (:status))"
	...
	@Query("SELECT c FROM Calendar2 c "
			...
			+ " AND c.clientId = :clientId AND c.status=:statusEnum "
			+ " AND c.active IS TRUE ANd c.isUnitCalendar is true"
	)
	List<Calendar2> getCalendar2toDate(Date date, Long clientId, CalendarStatusEnum statusEnum);
}
