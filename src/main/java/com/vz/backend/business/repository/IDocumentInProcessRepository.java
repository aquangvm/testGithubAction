...
public interface IDocumentInProcessRepository extends IRepository<DocumentInProcess> {
	...
	@Query("select d from DocumentInProcess d "
			+ " where d.docId = :docId and d.clientId =:clientId and d.active = true and (d.toUser = :userId OR d.delegaterId = :userId) and d.node = :node"
			+ " and d.step in (select max(d1.step) from DocumentInProcess d1 where d1.docId = :docId and d1.node = :node "
			+ " and d1.clientId =:clientId and d1.active = true and (d1.toUser = :userId OR d1.delegaterId = :userId) group by d1.toUser)")
