...
public interface INotificationRepository extends IRepository<Notification> {
	...
	@Query("SELECT COUNT(n) FROM Notification n WHERE n.id IN (SELECT max(t.id) FROM Notification t "
	...
	@Transactional
	@Modifying()
	@Query("UPDATE Notification n SET n.active = false WHERE n.docId = :docId AND n.docType = :docType")
	void deactiveAllByDocIdAndDocType(Long docId, DocumentTypeEnum docType);
	...
	@Transactional
	@Modifying()
	@Query("UPDATE Notification n SET n.read = :read WHERE n.docId = :docId and n.userId = :userId AND n.docType = :docType")
	void setReadByDocIdAndDocTypeAndUserId(boolean read, Long docId, DocumentTypeEnum docType, Long userId);
