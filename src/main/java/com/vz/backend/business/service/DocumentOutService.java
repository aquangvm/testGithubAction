...
import com.vz.backend.business.dto.ActionControlDto;
import com.vz.backend.business.dto.DataInitDto;
import com.vz.backend.business.dto.DocOutDto;
import com.vz.backend.business.dto.DocOutIssuedDto;
import com.vz.backend.business.dto.DocOutSignerDto;
import com.vz.backend.business.dto.DocumentOutDetailDto;
...
import com.vz.backend.business.dto.DocumentReplyDto;
...
import com.vz.backend.business.dto.SearchDocFormDto;
...
import com.vz.backend.business.dto.fullreport.SimpleProcess;
...
import com.vz.backend.core.dto.ReportDocumentDto;
...
public class DocumentOutService extends BaseService<DocumentOut> {
	...
	public void validateDocumentOutInput(DocumentOut input) {
		if (input == null || input.getDocTypeId() == null || StringUtils.isNullOrEmpty(input.getPreview()) || input.getUrgentId() == null
				|| input.getSecurityId() == null) {
...
////				String numberOrSign = input.getNumberInBook().toString() + input.getNumberOrSign();
////				input.setNumberOrSign(numberOrSign);
...
//					input.setNumberOrSign(input.getNumberInBook().toString());
//				}
//			}
//		} else {
			if (input.getNumberOrSign() != null) {
	...
	private List<DocumentReceive> validReceiveId(List<DocumentReceive> drList, Long docId) {
		...
		if (typeList == null || !typeList.contains(Constant.RECIEVE_TYPE_ORG)
				&& !typeList.contains(Constant.RECIEVE_TYPE_USER) && !typeList.contains(Constant.RECIEVE_TYPE_ALL)) {
		...
		drList.stream().forEach(i -> {
			if (Constant.RECIEVE_TYPE_ORG.equals(i.getType()) || Constant.RECIEVE_TYPE_ALL.equals(i.getType())) {
				if (orgIdList != null && !orgIdList.contains(i.getReceiveId())) {
					throw new RestExceptionHandler(Message.NOT_FOUND_ORG);
				}
			} else if (Constant.RECIEVE_TYPE_USER.equals(i.getType()) && (userIdList != null && !userIdList.contains(i.getReceiveId()))) {
	...
	private List<DocumentReceive> setDrList(List<DocumentReceive> drNewList, Long docId, int type) {
		...
		List<DocumentReceive> oldDrList = drService.findByClientIdAndDocIdAndType(BussinessCommon.getClientId(), docId,
				null);
		List<Long> allTypeList = oldDrList.stream().filter(j -> Constant.RECIEVE_TYPE_ALL.equals(j.getType()))
				.map(DocumentReceive::getReceiveId).collect(Collectors.toList());
		List<Long> orgTypeList = oldDrList.stream().filter(j -> Constant.RECIEVE_TYPE_ORG.equals(j.getType()))
				.map(DocumentReceive::getReceiveId).collect(Collectors.toList());
		List<Long> userTypeList = oldDrList.stream().filter(j -> Constant.RECIEVE_TYPE_USER.equals(j.getType()))
				.map(DocumentReceive::getReceiveId).collect(Collectors.toList());
		List<Long> orgTypeList2 = drNewList.stream().filter(j -> Constant.RECIEVE_TYPE_ORG.equals(j.getType()))
				.map(DocumentReceive::getReceiveId).collect(Collectors.toList());
		List<Long> userTypeList2 = drNewList.stream().filter(j -> Constant.RECIEVE_TYPE_USER.equals(j.getType()))
				.map(DocumentReceive::getReceiveId).collect(Collectors.toList());
		List<Long> allTypeList2 = drNewList.stream().filter(j -> Constant.RECIEVE_TYPE_ALL.equals(j.getType()))
				.map(DocumentReceive::getReceiveId).collect(Collectors.toList());
		...
		if (type == 1) {
			drNewList.stream().forEach(i -> {
				if (Constant.RECIEVE_TYPE_ORG.equals(i.getType()) && orgTypeList != null
						&& orgTypeList.contains(i.getReceiveId())
						|| Constant.RECIEVE_TYPE_USER.equals(i.getType()) && userTypeList != null
						&& userTypeList.contains(i.getReceiveId())
						|| Constant.RECIEVE_TYPE_ALL.equals(i.getType()) && allTypeList != null
			...
			oldDrList.stream().forEach(j -> {
				if (Constant.RECIEVE_TYPE_ORG.equals(j.getType()) && orgTypeList2 != null
						&& !orgTypeList2.contains(j.getReceiveId())
						|| Constant.RECIEVE_TYPE_USER.equals(j.getType()) && userTypeList2 != null
						&& !userTypeList2.contains(j.getReceiveId())
						|| Constant.RECIEVE_TYPE_ALL.equals(j.getType()) && allTypeList2 != null
	...
	public Boolean delete(Long id) {
		...
		if (dc.isPresent()) {
			...
			if (isCreator && !isSuperAdmin && !DocumentStatusEnum.DU_THAO.equals(doc.getStatus())
					&& !DocumentStatusEnum.BI_TRA_LAI.equals(doc.getStatus())) {
	...
	public boolean autoIssued(DocumentOut doc, Long nodeId) {
		try {
			...
			if (doc.getNumberInBook() == null) {
				...
				doc.setNumberOrSign(doc.getNumberInBook() + doc.getNumberOrSign());
			}
			doc.setNodeId(nodeId);
			...
			for (DocumentReceive dr : receiveIdList) {
				if (Constant.RECIEVE_TYPE_USER.equals(dr.getType())) {
	...
	public boolean issued(Long docId) {
		...
		try {
			...
			// Add notification
			notiService.addAll(userIds, docId, doc.getPreview(), DocumentTypeEnum.VAN_BAN_DI,
					NotificationHandleStatusEnum.DA_BAN_HANH, ModuleCodeEnum.DOCUMENT_IN_LIST);
	...
	public boolean issued(IssuedDto input) {
		...
		doc.setNumberOrSign(input.getNumberInBook()+input.getNumberOrSign());
		doc.setStatus(DocumentStatusEnum.DA_BAN_HANH);
	...
	public List<Long> getUserIdReceiveByDocId(List<DocumentReceive> receiveIdList, boolean isForward) {
		...
		for (DocumentReceive dr : receiveIdList) {
			if (dr.getType() == null)
				continue;
			switch (dr.getType()) {
			case Constant.RECIEVE_TYPE_USER:
				userIds.add(dr.getReceiveId());
				break;
			case Constant.RECIEVE_TYPE_ORG:
				orgIds.add(dr.getReceiveId());
				break;
			case Constant.RECIEVE_TYPE_ALL:
				orgIdGetAlls.add(dr.getReceiveId());
				break;
			case Constant.RECIEVE_TYPE_FORWARD:
				if(isForward) {
					userIds.add(dr.getReceiveId());
				}
				break;
		...
		for (int i = 0; i < listDocs.size(); i++) {
			...
			if (DocumentStatusEnum.CHO_BAN_HANH.equals(doc.getStatus()) && isVanThu && doc.getBookId() != null) {
				...
				List<DocumentReceive> receiveIdList = drService
						.findByClientIdAndDocIdAndType(BussinessCommon.getClientId(), doc.getId(), null);
				List<Long> listOrgIds = new ArrayList<>();
				List<Long> listUserIds = new ArrayList<>();
				for (DocumentReceive dr : receiveIdList) {
					if (Constant.RECIEVE_TYPE_USER.equals(dr.getType())) {
	...
	private List<DocumentReceive> getDataDrList(List<DocumentReceive> drList, Long id) {
		if (drList != null && !drList.isEmpty()) {
			...
			drList.stream().forEach(dr -> {
				if (Constant.RECIEVE_TYPE_USER.equals(dr.getType())) { // for choose person
...
//		}
	...
	public DataInitDto getDataInit() {
		...
		// get next bookId
		initDto.setBookId(dbookService.getMaxCurrentNumberByBookType(Constant.BOOK_DOC_VB_DI) + 1);

		// get book list
		List<DocumentBook> dbList = dbookService.findByActiveAndBookType(Constant.BOOK_DOC_VB_DI, true);
	...
	public ListObjectDto<DocOutIssuedDto> getListIssued(Boolean important, Long type, String text, String numberOrSignL,
			...
			Date startIssuedL, Date endIssuedL, Long docTypeL, Long docFieldsL, Long bookIdL, Pageable page) {
		User u = BussinessCommon.getUser();
		...
		if (type != null && type.longValue() == 2) { // tab vb trinh ki (da ban hanh)
			...
			if (type != null && type.longValue() == 1) { // tab vb trinh ki (da trinh ki)
				...
				docPage = docOutRepo.getListDocProcessAdvanceAssign(text, DocumentTypeEnum.VAN_BAN_DI, important,
						...
						endIssuedL, docTypeL, docFieldsL, bookIdL, docStatus, u.getClientId(), page);
			...
			docOutPage = docOutRepo.getListDocAdvance(text, clericalOrg, DocumentTypeEnum.VAN_BAN_DI, important, null,
					u.getId(), numberOrSignL, previewL, orgCreateNameL, startCreateL, endCreateL, startIssuedL,
					endIssuedL, docTypeL, docFieldsL, bookIdL, codes, u.getClientId(), u.getOrg(), page);
		}
...
//			dto.setRead(setRead(docId, objReadMap));
	...
	public ListObjectDto<DocOutIssuedDto> getListAll(String text, String outsideReceive, Boolean important,
			...
			Date endCreate, Date startIssued, Date endIssued, Long docTypeId, Long docFieldId, Long bookId,
			Pageable page) {
		User u = BussinessCommon.getUser();
		...
		Page<DocumentOut> doPage = docOutRepo.getListAllAdvance(new Date(), text, clericalOrg, outsideReceive,
				...
				docFieldId, bookId, u.getClientId(), signerDocIds, page);
	...
	public List<DocOutIssuedDto> exportExcel(String text, String outsideReceive,  Boolean important, String numberOrSign, String orgCreateName,
			...
			Long docTypeId, Long docFieldId, Long bookId, Sort sort) {
		List<DocumentOut> doPage = exportExcelToList(text, outsideReceive, important, numberOrSign, orgCreateName, personEnter, preview,
				startCreate, endCreate, startIssued, endIssued, docTypeId, docFieldId, bookId, sort);
	...
	public List<FindDocDto> exportExcelOut(String numberOrSign,
		...
		Long userVanThuId = userRepository.getUserVanThuBan("văn thư ban", orgService.getRootOrgId(u.getOrg()));
		List<String> userLeadNames = new ArrayList<>();
		userLeadNames.add("trưởng ban");
		userLeadNames.add("phó trưởng ban");
		...
		Long userBanThuBan = userRepository.getUserVanThuBan("văn thư ban", orgIdBan);
	...
	public List<DocumentOut> exportExcelToList(String text, String outsideReceive, Boolean important,
			String numberOrSign, String orgCreateName, String personEnter, String preview, Date startCreate,
			Date endCreate, Date startIssued, Date endIssued, Long docTypeId, Long docFieldId, Long bookId, Sort sort) {
		User u = BussinessCommon.getUser();
		boolean isLibrarian = userService.isVanThuVBDi(u);
		List<Long> signerDocIds = signerDocumentOut();
		return docOutRepo.getListAllAdvanceNotPaging(new Date(), text, clericalOrg, outsideReceive, important,
				...
				StringUtils.decodeFromUrl(orgCreateName), startCreate, endCreate, startIssued, endIssued, docTypeId,
				docFieldId, bookId, u.getClientId(), signerDocIds);
	}
...
//		Date endCreate = DateTimeUtils.handleSubmit(dto.getCreateTo(), Calendar.DAY_OF_MONTH, 1);
		...
		User u = BussinessCommon.getUser();
		List<Long> orgIds = orgService.orgAndSub(u.getOrg());
		List<Long> userVanThuId = userRepository.getUsersByNameCategory("văn thư ban", orgService.getRootOrgId(u.getOrg()));
		List<String> userLeadNames = new ArrayList<>();
		userLeadNames.add("trưởng ban");
		userLeadNames.add("phó trưởng ban");
		List<Long> listUser= clericalOrgRepo.getClericalOrgByOrgIdAndClientId(orgService.getRootOrgId(u.getOrg()),BussinessCommon.getClientId());
		...
		List<DocumentOut> docList = docOutRepo.findAllDoc(dto.getNumberOrSign(), dto.getPreview(), dto.getDocFieldsId(),
				dto.getStatus(), dto.getUrgentId(), dto.getSecurityId(), dto.getOrgIssuedId(), dto.getCreateFrom(), dto.getCreateTo(), dto.getBookId(),  u.getClientId(), orgIds);
		docList.addAll(docOutRepo.getFollowDocumentVTBBanHanh(dto.getNumberOrSign(), dto.getBookId(), dto.getPreview(), dto.getDocFieldsId(),
				dto.getStatus(), dto.getUrgentId(), dto.getOrgIssuedId(), dto.getCreateFrom(), dto.getCreateTo(), dto.getSecurityId(),u.getClientId(), userVanThuId, DocumentTypeEnum.VAN_BAN_DI, DocumentStatusEnum.DA_BAN_HANH));
		docList.addAll(docOutRepo.getFollowDocumentLeadBan(dto.getNumberOrSign(), dto.getBookId(), dto.getPreview(), dto.getDocFieldsId(),
				dto.getStatus(), dto.getUrgentId(), dto.getOrgIssuedId(), dto.getCreateFrom(), dto.getCreateTo(), dto.getSecurityId(), u.getClientId(), userLanhDaoBanIds, DocumentTypeEnum.VAN_BAN_DI, DocumentStatusEnum.DONE));
		...
		List<Long> orgIdBan= orgService.orgAndSub(orgService.getRootOrgId(BussinessCommon.getOrgId()));
		List<Long> userBanThuBan = userRepository.getUsersByNameCategory("văn thư ban", orgIdBan);
	...
	public List<DocumentOut> getByRemind() {
		...
		return docOutRepo.findByUserIdAndClientId(clericalOrg, docStatusId,
				u.isLead(), u.getOrg(), Constant.RECIEVE_TYPE_ORG, Constant.RECIEVE_TYPE_USER,
	...
	public DocumentOut forwardDoc(Long docId, Long main, List<Long> support, List<Long> show) {
		...
		if (main != null) {
			...
			docReceivers.add(findExistDocReceive(main, docId, Constant.RECIEVE_TYPE_FORWARD, HandleTypeEnum.MAIN));
		...
		if (!BussinessCommon.isEmptyList(support)) {
			for (Long userId : support) {
				...
				docReceivers
						.add(findExistDocReceive(userId, docId, Constant.RECIEVE_TYPE_FORWARD, HandleTypeEnum.SUPPORT));
		...
		if (!BussinessCommon.isEmptyList(show)) {
			for (Long userId : show) {
				...
				docReceivers
						.add(findExistDocReceive(userId, docId, Constant.RECIEVE_TYPE_FORWARD, HandleTypeEnum.SHOW));
	...
	public List<DocumentReceive> getReceivedUser(Long docId) {
		return docReceiveRepo.findByClientIdAndTypeAndDocId(BussinessCommon.getClientId(),
				Constant.RECIEVE_TYPE_FORWARD, docId);
	}
	...
	private List<Long> getUserOrListVanThuBan() {
		User user = BussinessCommon.getUser();
		Category positionVanThuBan = catService.findByName(user.getClientId(), "văn thư ban");
		...
		if (positionVanThuBan != null && user.getPositionModel() != null
			...
			usersCheck = userRepository.getUsersByNameCategory("văn thư ban", user.getOrg());
