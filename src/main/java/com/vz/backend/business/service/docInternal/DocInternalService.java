...
public class DocInternalService extends BaseService<DocumentInternal> {
	...
	public Long add(DocInternalCreateDto input) {
		...
		doc.setOrgCreateId(BussinessCommon.getOrgId());
		...
		if (DocumentStatusEnum.NB_CHO_DUYET.equals(input.getStatus())) {
			...
			if (input.getListUserApprove() != null)
				listUserIds.addAll(input.getListUserApprove());
	...
	public boolean update(Long id, DocInternalCreateDto input) {
		...
		doc.setSignerId(input.getSignerId());
		...
		// Update người duyệt và phòng ban đồng trình
		docInternalApproveService.update(doc.getId(), input.getListUserApprove(), input.getListOrgApprove(), input.getListCommenterApprove(), input.getSignerId());
		// Update nơi nhận
