...
public class DocumentService extends BaseService<Documents> {
    ...
    public Documents createDocument(Boolean receive, Documents doc, User u, Long clientId, Long orgReceiveId, Boolean createArrivalNumber) {
        ...
        //save file resolved
        try {
            if (isPhieuTrinh) this.saveResolveFile(doc);
        }catch (Exception e){
...
//			}
    ...
    private DocumentDetailDto setDocumentDetailDto(Documents document) {
        ...
        documentDetailDto.setFromDocuments(document);
    ...
    public List<Documents> findByClientIdAndBookId(Long clientId, Long bookId) {
        return docRepository.findByClientIdAndBookIdAndActiveTrueOrderByNumberArrivalDesc(clientId, bookId);
    }
    ...
    public ListObjectDto<DocumentDto> findProcessingDoc(Integer dayLeft, Integer typeHandle, Boolean expired, Boolean important,
        ...
        Category positionVanThuBan = categoryService.findByName(u.getClientId(), "văn thư ban");
        if (positionVanThuBan != null && u.getPositionModel() != null && (u.getPositionModel().getId().longValue() == positionVanThuBan.getId().longValue())) {
                userIds = userRepository.getUsersByNameCategory("văn thư ban", u.getOrg());
                if (userIds.isEmpty()) {
        ...
        if (!BussinessCommon.isEmptyPage(rsPage)) {
            ...
            Map<Long, List<NodeDto>> nextNodeIds = bpmnService.getNextNodes(nodeIds, TYPE_DOCUMENT.INCOMING);
        ...
        for (Documents doc : docList) {
            ...
            for (DocumentInProcess p : pfirstList) {
                if (p.getDocId().equals(doc.getId())) {
                    addProcesss(u, preNode, node, doc, p.getStep() + 1, uMain, uSupports, uShows, uDirections, comment, cmtContent, DocumentCommentTypeEnum.CHUYEN_XU_LY, files, deadline, Constant.START_STEP + 1, !hasMain, requestReview);
                }
            }
            // update node & status
            doc.setNode(node);
    ...
    private void addProcesss(User user, Long preNode, Long node, Documents doc, Integer stepsFirst, List<HandlerDto> main,
                             List<HandlerDto> supports, List<HandlerDto> shows, List<HandlerDto> directions, String comment, String cmtContent, DocumentCommentTypeEnum type, MultipartFile[] files, Date deadline, Integer transferStep, boolean hasMain, Boolean requestReview) {
        Long docId = doc.getId();

        // add process for new
        processService.saveList(user, preNode, node, main, HandleTypeEnum.MAIN, docId, stepsFirst, deadline, transferStep, hasMain, requestReview);
        ...
        if (!BussinessCommon.isEmptyList(main)) {
            ...
            if (!BussinessCommon.isEmptyList(assign)) {
                notiService.addAll(assign, docId, doc.getPreview(), DocumentTypeEnum.VAN_BAN_DEN,
                        NotificationHandleStatusEnum.XU_LY_CHINH, getByType(doc.getMergedLines(), ModuleCodeEnum.DOC_OUT_MAIN));
            }

            if (!BussinessCommon.isEmptyList(delegate)) {
                notiService.addAll(delegate, docId, doc.getPreview(), DocumentTypeEnum.VAN_BAN_DEN,
                        NotificationHandleStatusEnum.XU_LY_CHINH_UQ, getByType(doc.getMergedLines(), ModuleCodeEnum.DOC_OUT_MAIN));
            }
...
//				dto.setCanFinish(checkCanFinish(doc));
    ...
    public Page<FindDocDto> findAllDoc(FindDocDto dto, Pageable page) {
        ...
        List<Documents> documentsMap = docRepository.findAllIdDoc(dto.getExpired(), now, dto.getNumberOrSign(), dto.getBookId(), dto.getPreview(),
                ...
                dto.getDateIssuedFrom(), dto.getDateIssuedTo(), dto.getDateReceivedFrom(), dto.getDateReceivedTo(), dto.getPlaceSend(), orgIds, "văn thư ban", orgService.getRootOrgId(u.getOrg()));
        int no = 0;
        List<FindDocDto> rsList = new ArrayList<>();
...
//				&& !DocumentInHandleStatusEnum.DA_XU_LY_UQ.equals(oProcess.getHandleStatus()) &&
        ...
        addProcesss(frUser, preNode, node, doc, steps, uMain, uSupports, uShows, uDirections, comment, cmtContent, DocumentCommentTypeEnum.THEM_XU_LY, files, deadline, transferStep, false, false);
    ...
    /**
     * create file form template file -> save content in resolved file
     *
     * @param docId
     * @throws IOException
     */
    ...
    public Attachment saveResolveFile(Documents doc) {
        ...
        if (orgModel != null) {
            ...
            if (orgModel.getLevel() == 0) {
                ...
                dto.setPosition("Trưởng ban: " + dto.getOrgName());
                dto.setSinger(text);
                dto.setSingerUpper(text.toUpperCase());
            } else if (orgModel.getLevel() == 1) {
                text = "Trưởng phòng hành chính";
				text = (orgModel.getOrgConfigSign()!=null && orgModel.getOrgConfigSign().getUser()!=null)? orgModel.getOrgConfigSign().getUser().getPositionModel().getName():"";
                dto.setPosition("Lãnh đạo đơn vị: " + dto.getOrgName());
                dto.setSinger(text);
                dto.setSingerUpper(text.toUpperCase());
            } else dto.setPosition("");
        }
        if ("BAN CƠ YẾU CHÍNH PHỦ".equals(dto.getOrgNameUpper())) {
            dto.setOrgNameUpper("");
        }
        ...
        try {
            ...
            if (ftp) {
                ...
                try (InputStream inputStream = new FileInputStream(nFile)) {
    ...
    public Attachment updateResolveFile(Documents doc) {
        ...
        try {
            ...
            dto.setOrgNameUpper(orgModel != null ? orgModel.getName().toUpperCase() : "");
            if (orgModel != null) {
			String text = (orgModel.getOrgConfigSign()!=null && orgModel.getOrgConfigSign().getUser()!=null)? orgModel.getOrgConfigSign().getUser().getPositionModel().getName():"";
                if (orgModel.getLevel() == 0) {
                    dto.setPosition("Trưởng ban: " + dto.getOrgName());
                    dto.setSinger("Chánh văn phòng");
                } else if (orgModel.getLevel() == 1) {
    ...
    private Organization getOrgByCondition(Long receiveId, String type) {
        Organization org;
        if (Constant.RECIEVE_TYPE_USER.equals(type)) {
            User user = userService.valid(receiveId, Message.ERROR_SYS);
            org = user.getOrgModel();
        } else if (Constant.RECIEVE_TYPE_ORG.equals(type)) {
    ...
    public void saveFromDocOut(DocumentOut docOut, Long receiveId, String type) {
        ...
        boolean personalReceive = Constant.RECIEVE_TYPE_USER.equals(type);
    ...
    /**
     * CR : 9/3/2022
     * #2757 Xử lý bản mật chuyển xử lý đến đích danh
     *
     * @param parent
     * @param nodeId
     * @param users
    ...
    private void handleByConfidentialDoc2Receiver(List<Documents> parents, Long nodeId, List<User> users) {
        ...
        List<Condition> cs = bpmnService.getConditionByNodeId(nodeId);
    ...
    public List<Documents> transfer2(Long docId, String comment, String cmtContent, String[] main, String[] support, String[] show,
        ...
        // add process, tracking
        addProcesss(u, preNode, node, doc, oProcess.getStep() + 1, uMain, uSupports, uShows, uDirections, comment, cmtContent, DocumentCommentTypeEnum.CHUYEN_XU_LY, files, deadline, null, !hasMain, requestReview);
        this.all.clear();
        return docList;
    }
...
//		}
     ...
     * @param node
     * @param files
     * @param addOrg    : Đã chuyển đơn vị , muốn chuyển thêm đơn vị khác
     * @return
     */
...
//			attService.addListAttachment(doc.getAttachments(), dList);
    ...
    private List<Long> getUserOrListVanThuBan() {
        User user = BussinessCommon.getUser();
        Category positionVanThuBan = categoryService.findByName(user.getClientId(), "văn thư ban");
...
//		}
    ...
    public List<FindDocDto> exportExcels(Boolean expired, String numberOrSign, String preview,
        ...
        List<Documents> documentsMap = docRepository.findAllIdDoc(expired, now, numberOrSign, bookId, preview,
                ...
                startIssued, endIssued, startReceived, endReceived, orgIssuedName, orgIds, "văn thư ban", orgService.getRootOrgId(u.getOrg()));
        int no = 0;
        List<FindDocDto> rsList = new ArrayList<>();
        for (Documents documents : documentsMap) {
            FindDocDto rs = new FindDocDto(documents);
