...
//			}
			...
			Category positionVanThuBan = categoryService.findByName(BussinessCommon.getClientId(), "văn thư ban");
	...
	public ObjectNode getUserIdLeadBan() {
		...
		if ("chánh văn phòng".equals(currentUser.getPositionModel().getName().toLowerCase())) {
		...
		} else if ("trưởng ban".equals(currentUser.getPositionModel().getName().toLowerCase())) {
			userLeadNames.add("văn thư ban");
			List<Long> orgIds= orgService.orgAndSub(orgService.getRootOrgId(currentUser.getOrg()));
			List<Long> userBanThuBan = userRepository.getUsersByNameCategory("văn thư ban", orgIds);
