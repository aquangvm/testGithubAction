...
//			throw new RestExceptionHandler("Không tìm thấy file" + signature);
		...
		if(weekIn > week){
			title = "Lịch Công Tác Tuần Dự Kiến";
		}
		...
		try (InputStream template = getClass().getClassLoader().getResourceAsStream("calendar_template2.docx")) {
			if (template != null) {
	...
	public void setCalendarManyTime(List<Calendar2> all, int week) {
		List<Calendar2> calendar2ArrayList = new ArrayList<>();
		for (int i = 0; i < all.size(); i++) {
			long countDay = 0L;
			Calendar2 old = all.get(i);
			Calendar first = Calendar.getInstance();
			int nowWeek = first.get(Calendar.WEEK_OF_YEAR);
			if (nowWeek == week || week == DateTimeUtils.getWeekOfYear(old.getStartTime())) {
				long countDayStartToLastDayOfWeek = DateTimeUtils.daysBetween2DateTime(old.getStartTime(), DateTimeUtils.getDayLastDate(old.getStartTime()));
				long countDayStartToEnd = DateTimeUtils.daysBetween2DateTime(old.getStartTime(), old.getEndTime());
				if (countDayStartToLastDayOfWeek > countDayStartToEnd) {
					countDay = DateTimeUtils.daysBetween2DateTime(old.getStartTime(), old.getEndTime());
				} else {
					countDay = DateTimeUtils.daysBetween2DateTime(old.getStartTime(), DateTimeUtils.getDayLastDate(old.getStartTime()));
				}
				if (countDay > 0) {
					for (int index = 1; index <= countDay; index++) {
						Date nextDay = DateTimeUtils.getDayAfter(old.getStartTime(), index);
						Calendar2 calendar2 = new Calendar2();
						calendar2.setCalendater(false, old);
						calendar2.setStartTime(new Timestamp(nextDay.getTime()));
						setCalendar(calendar2, old);
						calendar2ArrayList.add(calendar2);
					}
				}
				if (DateTimeUtils.getHour(old.getStartTime()) > DateTimeUtils.getHour(old.getEndTime()) && DateTimeUtils.daysBetween2DateTime(DateTimeUtils.getDayFirstDate(old.getEndTime()), DateTimeUtils.getDayLastDate(old.getStartTime())) > 0) {
					Calendar2 calendar2 = new Calendar2();
					calendar2.setCalendater(false, old);
					calendar2.setStartTime(old.getEndTime());
					setCalendar(calendar2, old);
					calendar2ArrayList.add(calendar2);
				}
			} else {
				Date startTimeBetween = DateTimeUtils.getDayFirstDate(old.getEndTime());
				countDay = DateTimeUtils.daysBetween2DateTime(startTimeBetween, old.getEndTime());
				if (countDay > 0) {
					for (int index = 1; index <= countDay; index++) {
						Date nextDay = DateTimeUtils.getDayAfter(startTimeBetween, index);
						Calendar2 calendar2 = new Calendar2();
						calendar2.setCalendater(false, old);
						calendar2.setStartTime(new Timestamp(nextDay.getTime()));
						setCalendar(calendar2, old);
				...
				} else if (DateTimeUtils.getHour(old.getStartTime()) > DateTimeUtils.getHour(old.getEndTime())) {
					old.setStartTime(old.getEndTime());
				}
			}
		}
		all.addAll(calendar2ArrayList);
	}

	public void setCalendar(Calendar2 calendar2, Calendar2 old) {
		calendar2.setId(old.getId());
		calendar2.setStatus(old.getStatus());
		calendar2.setCreateBy(old.getCreateBy());
		calendar2.setCreateUserName(old.getCreateUserName());
		calendar2.setUnitCalendar(old.isUnitCalendar());
		calendar2.setRegisterBan(old.getRegisterBan());
	}

	public List<String> getOrgLevel(User user, int orgType) {
		if (orgType == Constant.CALENDAR_ORG_TYPE_BAN && orgService.isUserOfOrgType(user, Constant.BAN)) {
			return Arrays.asList(Constant.BAN);
		}
		return null;
	}
...
//			return false;
	...
	public List<Calendar2> getCalendar2toDate(String date) {
		Date dateSelect;
		try {
		...
		return calendar2Repository.getCalendar2toDate(dateSelect, BussinessCommon.getClientId(), CalendarStatusEnum.APPROVE);
	}
